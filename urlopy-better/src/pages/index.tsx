...
//     undefined, // no input
//     { enabled: sessionData?.user.role !== undefined }
//   );
...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-neutral-900 text-center font-semibold text-white">
      <div className="text-4xl">Urlopy</div>
      {session.data ? (
        <>
          <div className="text-4xl">Zalogowany</div>
          <Button onClick={() => {signOut().catch(e => console.log(e))}}>Wyloguj</Button>
        </>
      ) : (
        <>
          <div className="text-4xl">Nie zalogowany</div>
          <Button onClick={() => {signIn().catch(e => console.log(e))}}>Zaloguj</Button>
        </>
      )}
    </div>
  );
